<nav x-data="{ open: false }" class="bg-white border-b fixed w-full z-50">
	...
	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
		<div class="flex justify-between h-16">
			<div class="flex">
				...
				<!-- Navigation Links -->
				<div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
					<x-nav-link :href="route('order.create')" :active="request()->routeIs('order.create')"> {{ __('登録') }} </x-nav-link>
				</div>
				<div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex items-center">
					<x-dropdown> <x-slot name="trigger"> <x-nav-link>{{ __('一覧') }}</x-nav-link> </x-slot> <x-slot name="content"> <x-dropdown-link :href="route('foods.index')" :active="request()->routeIs('food.index')">食材</x-dropdown-link> <x-dropdown-link
						:href="route('order.index')" :active="request()->routeIs('order.index')">購入品</x-dropdown-link> </x-slot> </x-dropdown>
				</div>

				<div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
					<x-nav-link :href="route('calc')" :active="request()->routeIs('calc')"> {{ __('計算') }} </x-nav-link>
				</div>
			</div>
	...
	<div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden bg-white z-30">
