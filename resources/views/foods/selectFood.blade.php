                 ...
                 <div class="w-full md:w-1/2 lg:w-1/3 px-4">
		         	<form action="{{ route('order.store') }}" method="POST">
                		...
                		<div class="bg-white rounded-xl relative z-10 overflow-hidden border border-green-900 border-opacity-20 shadow-pricing py-10 px-8 sm:p-12 lg:py-10 lg:px-6 xl:p-12 mb-10">
                       		<span class="text-body-color font-semibold text-lg block mb-4">
                          		{{ $food->food_code }}
                       		</span>
                       		<p class="font-bold text-green-700 mb-5 text-[4vw] md:text-[2vw]">
                          		{{ $food->food_name }}
                       		</p>
                           <p class="text-base text-body-color pb-8 mb-8 border-b border-[#F2F2F2]">
                              「{{ $food->food_name }}」の主な栄養価は以下の通りです。
                           </p>
                           <div class="mb-7">
                              <p class="text-base text-body-color leading-loose mb-1">
                                 エネルギー：{{ $food->ENERC_KCAL }}kcal
                              </p>
                              <p class="text-base text-body-color leading-loose mb-1">
                                 水分：{{ $food->WATER }}g
                              </p>
                           ...
                           <button type="submit" name="addExec" class="w-full block text-base font-semibold text-green-700 bg-transparent border border-[#D4DEFF] rounded-md text-center p-4 hover:text-white hover:bg-green-700 hover:border-green-700 transition">
                              追加する
                           </button>
                           <div>
